...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
			...
			case 'TBC-stage':
				{
						defaultCamZoom = 0.8;
						curStage = 'TBC-stage';
						var bg:FlxSprite = new FlxSprite(-450.6, -119.8).loadGraphic(Paths.image('bg'));
						bg.x += 60;
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
		...
		monster = new Character(100, 100, 'monster');
		mom = new Character(100, 100, 'mom');
		pico = new Character(100, 100, 'pico');
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if (ismonster)
							{
								monster.playAnim('sing' + dadsDir[daNote.noteData], true);
								monster.holdTimer = 0;
							}
						if (ismom)
							{
								mom.playAnim('sing' + dadsDir[daNote.noteData], true);
								mom.holdTimer = 0;
							}
						if (ispico)
							{
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					switch(SONG.song.toLowerCase()) //จบเพลงสุดท้าย
					{
						//case 'jesus lol':
						//    LoadingState.loadAndSwitchState(new VideoState("assets/videos/jesus.webm", new PlayState()));
						default:
							FlxG.sound.playMusic(Paths.music('freakyMenu'));
				...
				else
				{
					...
					switch(SONG.song.toLowerCase()) //จบเพลงแต่ไม่ใช่สุดท้าย
				    {
						//case 'jesus lol':
						//    LoadingState.loadAndSwitchState(new VideoState("assets/videos/jesus.webm", new PlayState()));
					    default:
	...
	private function popUpScore(daNote:Note):Void
		{
			if (FlxG.save.data.NoteSplash){
				...
				if (!curStage.startsWith('school'))
				{
					var tex:flixel.graphics.frames.FlxAtlasFrames = Paths.getSparrowAtlas('noteSplashes/noteSplashes', 'shared');
					sploosh.frames = tex;
				...
				{
					sploosh.loadGraphic(Paths.image('noteSplashes/noteSplashes-pixels', 'shared'), true, 50, 50);
	...
	*/
	...
	override function stepHit()
	{
		...
		*/
		if (SONG.song.toLowerCase() == 'tbc')
			switch (curStep)
			{
				case 58 | 73 | 105 | 137 | 169 | 201 | 223 | 249 | 273 | 351 | 377 | 413 | 442 | 457 | 489 | 521 | 553 | 561 | 585 | 617:
					defaultCamZoom = 0.75;
				case 63 | 82 | 112 | 142 | 239 | 255 | 281 | 355 | 418 | 466 | 496 | 527 | 590 | 625:
					defaultCamZoom = 0.9;
				case 96 | 125 | 161 | 192 | 205 | 224 | 257 | 289 | 369 | 384 | 448 | 480 | 513 | 545 | 576 | 608:
					defaultCamZoom = 0.8;
				case 153 | 154 | 156 | 157 | 177 | 179 | 181 | 183 | 217 | 219 | 221 | 241 | 366 | 367 | 401 | 425 | 431 | 432 | 441 | 473 | 475 | 476 | 477 | 505 | 529 | 537 | 539 | 541 | 601 | 603 | 604 | 605 | 633 | 673 | 705 | 736 | 800 | 816 | 833 | 849 | 864 | 881 | 897:
	...
	function changeBfCharacternoflip(id:String)
		{				
			var oldbfx = boyfriend.x;
			var oldbfy = boyfriend.y;
			remove(boyfriend);
